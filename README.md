...
**Overall: 2× faster than GNU coreutils** (101% faster overall; geometric mean of 91 timed tools · 90 wins · median 1.89×).
<!-- bench-headline:end -->
...
<!-- bench-table:start -->
_CI / suite bench · `2026-07-24T15:31:42Z` · N=15 median · x86_64 · Linux 6.17.0-1020-azure_

| Tool | Command | GNU | f00 `--core` | vs GNU |
|------|---------|-----|--------------|--------|
| `true` | `f00-true --core` | 0.41 ms | **0.17 ms** | **~2.4×** |
| `basename` | `f00-basename --core /usr/bin/ls` | 0.57 ms | **0.17 ms** | **~3.4×** |
| `nproc` | `f00-nproc --core` | 0.58 ms | **0.23 ms** | **~2.6×** |
| `whoami` | `f00-whoami --core` | 0.68 ms | **0.22 ms** | **~3.1×** |
| `cat` | `f00-cat --core fixture.txt` | 0.59 ms | **0.20 ms** | **~2.9×** |
| `wc` | `f00-wc --core -l fixture.txt` | 0.63 ms | **0.35 ms** | **~1.8×** |
| `md5sum` | `f00-md5sum --core fixture.txt` | 0.90 ms | **0.39 ms** | **~2.3×** |
| `sha256sum` | `f00-sha256sum --core fixture.txt` | 0.95 ms | **0.46 ms** | **~2.1×** |
| `sort` | `f00-sort --core fixture.txt` | 1.11 ms | **0.81 ms** | **~1.4×** |
| `ls` | `f00-ls --core -1 dir` | 0.73 ms | **0.47 ms** | **~1.6×** |
<!-- bench-table:end -->
...
```bash
# pin version
curl -fsSL https://f00.sh/install.sh | F00_VERSION=v0.15.15 bash
...
Release assets for `v0.15.15` include tarball, **deb**, **rpm**, and **Arch** packages.
...
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.15/f00_0.15.15_amd64.deb
sudo dpkg -i f00_0.15.15_amd64.deb

# Fedora / RHEL example
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.15/f00-0.15.15-1.x86_64.rpm
sudo rpm -Uvh f00-0.15.15-1.x86_64.rpm

# Arch example (release package)
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.15/f00-0.15.15-1-x86_64.pkg.tar.zst
sudo pacman -U f00-0.15.15-1-x86_64.pkg.tar.zst
```
...
```text
░▒▓████████████████████████████████████████████▓▒░░░
█▓▒░  f 0 0 t i l s  ·  scene card  ·  v0.15.15 ░▒▓█ 
████████████████████████████████████████████████████
█  ▄████████▄   ▄███████▄   ▄███████▄              █
█  ███▀▀▀▀███   ███▀▀▀▀███  ███▀▀▀▀███  freest.    █
█  ███        ▄ ███     ███ ███     ███  ASM       █
