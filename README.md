...
**Overall: 2.6× faster than GNU coreutils** (164% faster overall; geometric mean of 91 timed tools · 90 wins · median 2.61×).
<!-- bench-headline:end -->
...
<!-- bench-table:start -->
_CI / suite bench · `2026-07-24T15:21:03Z` · N=15 median · x86_64 · Linux 6.17.0-1020-azure_

| Tool | Command | GNU | f00 `--core` | vs GNU |
|------|---------|-----|--------------|--------|
| `true` | `f00-true --core` | 0.54 ms | **0.25 ms** | **~2.2×** |
| `basename` | `f00-basename --core /usr/bin/ls` | 0.82 ms | **0.25 ms** | **~3.3×** |
| `nproc` | `f00-nproc --core` | 0.83 ms | **0.26 ms** | **~3.2×** |
| `whoami` | `f00-whoami --core` | 0.88 ms | **0.27 ms** | **~3.3×** |
| `cat` | `f00-cat --core fixture.txt` | 0.85 ms | **0.30 ms** | **~2.8×** |
| `wc` | `f00-wc --core -l fixture.txt` | 0.87 ms | **0.37 ms** | **~2.4×** |
| `md5sum` | `f00-md5sum --core fixture.txt` | 1.26 ms | **0.39 ms** | **~3.2×** |
| `sha256sum` | `f00-sha256sum --core fixture.txt` | 1.22 ms | **0.45 ms** | **~2.7×** |
| `sort` | `f00-sort --core fixture.txt` | 1.34 ms | **0.76 ms** | **~1.8×** |
| `ls` | `f00-ls --core -1 dir` | 1.04 ms | **0.42 ms** | **~2.5×** |
<!-- bench-table:end -->
...
```bash
# pin version
curl -fsSL https://f00.sh/install.sh | F00_VERSION=v0.15.14 bash
...
Release assets for `v0.15.14` include tarball, **deb**, **rpm**, and **Arch** packages.
...
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.14/f00_0.15.14_amd64.deb
sudo dpkg -i f00_0.15.14_amd64.deb

# Fedora / RHEL example
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.14/f00-0.15.14-1.x86_64.rpm
sudo rpm -Uvh f00-0.15.14-1.x86_64.rpm

# Arch example (release package)
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.14/f00-0.15.14-1-x86_64.pkg.tar.zst
sudo pacman -U f00-0.15.14-1-x86_64.pkg.tar.zst
```
...
```text
░▒▓████████████████████████████████████████████▓▒░░░
█▓▒░  f 0 0 t i l s  ·  scene card  ·  v0.15.14 ░▒▓█ 
████████████████████████████████████████████████████
█  ▄████████▄   ▄███████▄   ▄███████▄              █
█  ███▀▀▀▀███   ███▀▀▀▀███  ███▀▀▀▀███  freest.    █
█  ███        ▄ ███     ███ ███     ███  ASM       █
