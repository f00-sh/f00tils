...
#
# Beta pin example:
#   curl -fsSL https://f00.sh/install.sh | F00_VERSION=v0.16.8 bash
#
# Env:
#   INSTALL_DIR      default ~/.local/bin
#   F00_VERSION      release tag (default: latest; default: latest / v0.16.8)
...
TOOLS_ALL=(
  ...
  base64 basenc base32 dircolors chroot stty stdbuf runcon chcon config grep egrep fgrep find xargs diff cmp diff3 sdiff
)
...
main() {
  printf "\n${BOLD}f00tils${RESET} ${DIM}installer · binary f00 · 0.16.8 multicall${RESET}\n" >&2
  ...
  printf "${DIM}GNU drop-in: export F00_CORE=1  (or core=true / --core — not auto for scripts)${RESET}\n" >&2
  printf "${DIM}knobs: F00_TOOLS=all|ls,cat,find,…  F00_SUPERSEDE=0  F00_LOCAL=asm  F00_VERSION=v0.16.8${RESET}\n" >&2
...
theme = terminal
# core = false (default): modern default. Scripts do NOT auto-enable --core.
# For GNU-identical PATH drop-in / CI: core = true  OR  export F00_CORE=1  OR  tool --core
core = false
color = auto
icons = auto
animations = true
